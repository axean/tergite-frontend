...
def test_read_job(db, client, job_id: str):
...
@pytest.mark.parametrize("job_id", _JOB_IDS)
def test_read_job_result(db, client, job_id: str):
...
@pytest.mark.parametrize("job_id", _JOB_IDS)
def test_read_job_download_url(db, client, job_id: str):
    """Get to /jobs/{job_id}/download_url the job download_url for the given job_id"""
    insert_in_collection(database=db, collection_name=_COLLECTION, data=_JOBS_LIST)

    response = client.get(f"/jobs/{job_id}/download_url")
    got = response.json()
    expected_job = list(filter(lambda x: x["job_id"] == job_id, _JOBS_LIST))[0]
...
def test_create_job(db, client, backend: str):
...
def test_read_jobs(db, client, nlast: int):
    ...
    response = client.get(f"/jobs{query_string}")
...
def test_update_job_result(db, client, job_id: str, memory: list):
    """PUT to /jobs/{job_id}/result updates the result of the job with the given memory object"""
    insert_in_collection(database=db, collection_name=_COLLECTION, data=_JOBS_LIST)

    response = client.put(f"/jobs/{job_id}/result", json=memory)
    got = response.json()
    expected_job = list(filter(lambda x: x["job_id"] == job_id, _JOBS_LIST))[0]
...
def test_update_job_status(db, client, job_id: str, status: str):
    """PUT to /jobs/{job_id}/status updates the status of the job of the given job id"""
    insert_in_collection(database=db, collection_name=_COLLECTION, data=_JOBS_LIST)

    response = client.put(f"/jobs/{job_id}/status", json=status)
    got = response.json()
    expected_job = list(filter(lambda x: x["job_id"] == job_id, _JOBS_LIST))[0]
...
def test_update_job_download_url(db, client, job_id: str, url: str):
    """PUT to /jobs/{job_id}/download_url updates the download_url of the job of the given job id"""
    insert_in_collection(database=db, collection_name=_COLLECTION, data=_JOBS_LIST)

    response = client.put(f"/jobs/{job_id}/download_url", json=url)
    got = response.json()
    expected_job = list(filter(lambda x: x["job_id"] == job_id, _JOBS_LIST))[0]
...
def test_update_timelog_entry(db, client, job_id: str, event_name: str):
    """PUT to /jobs/{job_id}/timelog refreshes the timelog of the given event of the job of the given job id"""
    insert_in_collection(database=db, collection_name=_COLLECTION, data=_JOBS_LIST)

    response = client.post(f"/jobs/{job_id}/timelog", json=event_name)
    got = response.json()
    expected_job = list(filter(lambda x: x["job_id"] == job_id, _JOBS_LIST))[0]
