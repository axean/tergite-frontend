...
def test_read_calibrations(nlast, db, client):
...
def test_read_calibration(job_id: str, db, client):
    """Get `/calibrations/{job_id}` reads the calibration of the given job_id"""
    insert_in_collection(
        database=db, collection_name=_COLLECTION, data=_CALIBRATIONS_LIST
    )

    response = client.get(f"/calibrations/{job_id}")
    got = response.json()
...
def test_create_calibrations(db, client):
