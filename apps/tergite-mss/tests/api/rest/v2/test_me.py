...
def test_view_own_projects_in_less_detail(
    ...
    with client as client:
        ...
        project_list = [
            ...
            # FIXME: Right now, projects still have "user_emails" instead of "user_ids"
...
@pytest.mark.parametrize("user_email, cookies, project", _MY_PROJECT_REQUESTS)
def test_view_my_project_in_less_detail(
    user_email, cookies, project, client, inserted_projects
):
...
@pytest.mark.parametrize("user_email, cookies, project", _OTHERS_PROJECT_REQUESTS)
...
def test_view_my_app_token_in_less_detail(
    user_id, cookies, token, client, inserted_projects, inserted_app_tokens, freezer
):
...
@pytest.mark.parametrize("user_id, cookies", _USER_ID_COOKIES_FIXTURE)
def test_read_all_my_jobs(db, client, user_id, cookies):
    ...
    with client as client:
        ...
        expected = order_by(
            [job for job in _JOBS_LIST if job["user_id"] == user_id], field="job_id"
        )
...
def test_read_all_my_jobs_of_given_project(db, client, user_id, cookies, project):
    ...
    with client as client:
        response = client.get(
            f"/v2/me/jobs", cookies=cookies, params={"project_id": project.id}
        )
        got = order_by(response.json(), field="job_id")
        expected = order_by(
            [
                ...
                if job["user_id"] == user_id and job["project_id"] == project.id
            ],
            field="job_id",
        )
